...
po::variables_map parse_mask_command(po::parsed_options parsed) {
    ...
    filt_desc.add_options()("input-mat,i", po::value<std::string>()->required(),
        "Input mutation-annotated tree file [REQUIRED]")("output-mat,o",
        po::value<std::string>()->required(),
        "Path to output masked mutation-annotated tree file [REQUIRED]")("simplify,S",
        po::bool_switch(),
        "Use to automatically remove identifying information from the tree, including all sample "
        "names and private mutations.")("restricted-samples,s",
        po::value<std::string>()->default_value(""),
        "Sample names to restrict. Use to perform masking")("rename-samples,r",
        po::value<std::string>()->default_value(""),
    ...
    try {
        po::store(po::command_line_parser(opts).options(filt_desc).run(), vm);
        po::notify(vm);
...
void mask_main(po::parsed_options parsed) {
    ...
    if ((simplify) & (rename_filename != "")) {
        ...
        fprintf(stderr, "ERROR: Sample renaming and simplification are mutually exclusive "
                        "operations. Review argument choices\n");
        exit(1);
    }
...
void simplify_tree(MAT::Tree *T) {
    ...
    */
    ...
    std::shuffle(all_leaves.begin(), all_leaves.end(), rng);
    int rid = 0;
